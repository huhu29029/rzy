...
func Register(u model.SysUser) (err error, userInter model.SysUser) {
	...
	global.DB.Where("username = ? AND password = ?", u.Username, u.Password).Preload("Authority").First(&user)
	return err, user
}
...
func FindUserById(id int) (err error, user *model.SysUser) {
	var u model.SysUser
	err = global.DB.Where("`id` = ?", id).Preload("Authority").First(&u).Error
	return err, &u
}
...
func IsAdmin(authorityId uint) bool {
	if authorityId == 1 {
		return true
	} else {
		return false
...
func IsPreacher(authorityId uint) bool {
	if authorityId == 3 {
		return true
	} else {
		return false
