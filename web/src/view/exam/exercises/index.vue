<template>
  <div>
    <el-row v-if="this.questions.length !== 0">
      <el-col>
        <h1>{{ examTitle }}</h1>
        <p>共包含{{ questions.length }}道推荐题目</p>
      </el-col>
      <el-col :span="18">
    ...
    <el-empty v-else description="暂无推荐题目"></el-empty>
  </div>
</template>
...
export default {
  ...
  async created() {
    const { examTitle, questions } = this.$route.params;
  ...
  methods: {
    ...
    async getQuestion() {
      if (this.questionIdx === this.questions.length - 1) {
        this.$message.info('推荐题目已做完！');
        return;
      }
      const questionId = this.questions[this.questionIdx];
      this.questionIdx += 1;
