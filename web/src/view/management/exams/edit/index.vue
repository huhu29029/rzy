<template>
  <div>
      ...
      <div>
        <p>* 题目选择</p>
        <el-table :data="selectedQuestions" stripe max-height="500" style="width: 100%">
...
<script>
import { getExam, editExam } from '@/api/exam.js';
...
export default {
  ...
  async created() {
    const res = await getExam(this.$route.query.id);
    const { exam } = res.data;
    this.examForm.title = exam.title;
    this.selectedQuestions = exam.questions.reduce((acc, cur) => {
      acc.push(cur.id);
      return acc;
    }, []);
  },
  methods: {
    ...
    async submit() {
      ...
      try {
        const res = await editExam(this.$route.query.id, {
        ...
        this.$message.success('编辑试卷成功！,3s后跳转至试卷列表页...');
        setTimeout(() => {
          this.$router.push({
            name: 'm-exams',
          });
