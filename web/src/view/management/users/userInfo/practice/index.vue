...
<script>
import { getQuestionResult, getUserInfoQuestionsHistory } from '@/api/questions.js';
...
export default {
  ...
  methods: {
    ...
    async getQuestionsHistory(
      ...
      const res = await getUserInfoQuestionsHistory(this.id, page, size);
      const { data } = res;
      ...
      for (let i = 0; i < questionHistory.length; i++) {
        ...
        table.push({
          id: question.id,
          date: formatTimeToStr(question.startTime),
          title: questionResult.title,
          result: questionResult.isRight ? '对' : '错',
        });
      }
